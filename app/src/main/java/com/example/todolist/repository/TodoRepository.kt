...
class TodoRepository private constructor(context: Context){
    //먼저 Room.databaseBuilder().build() 를 통해 데이터베이스를 빌드함
    ...
    //companion object 객체는 클래스가 생성될 때 메모리에 적재되면서 동시에 생성하는 객체로, 데이터베이스 생성 및 초기화를 담당하기 위해 작성함
    companion object{
        ...
        fun get(): TodoRepository{
            return INSTANCE ?:
            throw IllegalStateException("TodoRepository must be initalized")
        }
    }
